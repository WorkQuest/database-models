...
exports.ratingStatusCanInviteMeOnQuestSchema = Joi.number().valid(...Object.keys(models_1.RatingStatus).map(key => parseInt(key)).filter(key => !isNaN(key))).example(models_1.RatingStatus.AllStatuses).label("ProfileVisibilityNetwork");
exports.ratingStatusCanRespondOnQuestSchema = Joi.number().valid(...Object.keys(models_1.RatingStatus).map(key => parseInt(key)).filter(key => !isNaN(key))).example(models_1.RatingStatus.AllStatuses).label("ProfileVisibilityStatus");
exports.ratingStatusInMySearchSchema = Joi.number().valid(...Object.keys(models_1.RatingStatus).map(key => parseInt(key)).filter(key => !isNaN(key))).example(models_1.RatingStatus.AllStatuses).label("ProfileVisibilityNetwork");
