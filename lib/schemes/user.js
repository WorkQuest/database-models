...
exports.networkProfileVisibilitySchema = Joi.number().valid(...Object.keys(models_1.NetworkProfileVisibilityType).map(key => parseInt(key)).filter(key => !isNaN(key))).example(models_1.NetworkProfileVisibilityType.EveryoneOnTheInternet).label("NetworkProfileVisibility");
