...
exports.userRaiseViewStatusSchema = Joi.number().valid(...Object.keys(models_1.UserRaiseStatus).map(key => parseInt(key)).filter(key => !isNaN(key))).example(models_1.UserRaiseStatus.Unpaid).label('UserRaiseViewStatus');
